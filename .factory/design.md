...
Generation: Azure AI Foundry factory image deployment via `/opt/fleet/lib/gen-image.sh`, 2026-08-27. The selected original and its exact prompt sidecar live in `assets/src/`; optimized WebP derivatives live in `public/assets/`. The 1200 × 630 social card is a cropped, compressed derivative of that same original. It has no added text and is used only for page previews. Generated imagery is disclosed in the site footer.

Icons are original inline SVG marks built from staff lines, cue brackets, and simple geometric controls. The 180 px apple-touch PNG is a raster rendering of the same original staff-and-cobalt mark. They use `currentColor`, and informative icons have accessible names while decorative icons are hidden.
